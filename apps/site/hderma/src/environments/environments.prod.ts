export const environment = {
  production: false,
  BaseImage:"https://images.tazagroup.vn/",
  APITINYMCE: "1cdi3qs7qw7nogvpu6poxqc6z7bf4a4hurwyao0kdbd741dl",
  APIURL:"https://api.hderma.vn",
  SECRETKEY: "mycustomuselongsecret",
  EXPIRESIN: "60 days",
    FirebaseInit:{
    apiKey: "AIzaSyAuvMdAWsbDXzA6IoOIiuRhr8w6-VqN4gs",
    authDomain: "hderma-e806b.firebaseapp.com",
    projectId: "hderma-e806b",
    storageBucket: "hderma-e806b.appspot.com",
    messagingSenderId: "297498165872",
    appId: "1:297498165872:web:2636d766b29bc99515eab5",
    measurementId: "G-ZX7TQ6ZTTH"
  },
  VNPAY:{
    vnp_Amount:0,
    vnp_Command:'pay',
    vnp_CreateDate:20210801153333,
    vnp_CurrCode:'VND',
    vnp_IpAddr:'127.0.0.1',
    vnp_Locale:'vn',
    vnp_OrderInfo:'Test 1',
    vnp_OrderType:'other',
    vnp_ReturnUrl:'http://localhost:4200/camon',
    vnp_TmnCode:'HDRMA001',
    vnp_TxnRef:'DH1',
    vnp_Version:'2.1.0',
  },
  VNPAYTEST:{
    vnp_Amount:0,
    vnp_Command:'pay',
    vnp_CreateDate:20210801153333,
    vnp_CurrCode:'VND',
    vnp_IpAddr:'127.0.0.1',
    vnp_Locale:'vn',
    vnp_OrderInfo:'Test 1',
    vnp_OrderType:'other',
    vnp_ReturnUrl:'http://localhost:4200/camon',
    vnp_TmnCode:'HDERMAWT',
    vnp_TxnRef:'DH1',
    vnp_Version:'2.1.0',
  },
  VNPAYURL:'https://pay.vnpay.vn/vpcpay.html?',
  VNPAYURLTEST:'https://sandbox.vnpayment.vn/paymentv2/vpcpay.html?',
  vnp_HashSecret:"PYRVEIXBCKJTCAQFDPBQXYPSTXTACKUZ",
  vnp_HashSecretTEST:"RPEVGGQRQAJTRFYAYKDSDIEWANYGICRT",
  keydriveapi: {
    "type": "service_account",
    "project_id": "tazagroup",
    "private_key_id": "83d6cbbf8e0cb05f0b043d3e0a1958554dd590bc",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "drive-tazagroup@tazagroup.iam.gserviceaccount.com",
    "client_id": "107824773839130289254",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/drive-tazagroup%40tazagroup.iam.gserviceaccount.com"
  },
  DB_USERS: {
    type: 'mysql',
    host: '103.221.222.71',
    port: 3306,
    username: 'tazaspac_chikiet',
    password: '@Hikiet88',
    database: 'tazaspac_tazagroup_users',
    autoLoadEntities: true,
    synchronize: true,
    charset: "utf8mb4",
    collation: "utf8mb4_unicode_ci",
  },
  DB_HDERMA: {
    name:'hderma',
    type: 'mysql',
    host: '103.221.222.71',
    port: 3306,
    username: 'tazaspac_chikiet',
    password: '@Hikiet88',
    database: 'tazaspac_tazagroup_hderma',
    autoLoadEntities: true,
    synchronize: true,
    charset: "utf8mb4",
    collation: "utf8mb4_unicode_ci"
  },
  DB_TIMONA: {
    name:'timona',
    type: 'mysql',
    host: '103.221.222.71',
    port: 3306,
    username: 'tazaspac_chikiet',
    password: '@Hikiet88',
    database: 'tazaspac_tazagroup_timona',
    autoLoadEntities: true,
    synchronize: true,
    charset: "utf8mb4",
    collation: "utf8mb4_unicode_ci"
  },
  DB_DAOTAO: {
    name:'daotao',
    type: 'mysql',
    host: '103.221.222.71',
    port: 3306,
    username: 'tazaspac_chikiet',
    password: '@Hikiet88',
    database: 'tazaspac_tazagroup_daotao',
    autoLoadEntities: true,
    synchronize: true,
    charset: "utf8mb4",
    collation: "utf8mb4_unicode_ci"
  },
  DB_QLHOCVIEN: {
    name:'qlhocvien',
    type: 'mysql',
    host: '103.221.222.71',
    port: 3306,
    username: 'tazaspac_chikiet',
    password: '@Hikiet88',
    database: 'tazaspac_tazagroup_qlhocvien',
    autoLoadEntities: true,
    synchronize: true,
    charset: "utf8mb4",
    collation: "utf8mb4_unicode_ci"
  },
};