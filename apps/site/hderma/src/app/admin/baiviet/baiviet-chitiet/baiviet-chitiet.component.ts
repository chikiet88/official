...
export class BaivietChitietComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._BaivietService.getDanhmucs().subscribe();
  ...
  configTiny1: EditorComponent['init'] = {
    ...
    toolbar: 'undo redo |fontfamily fontsize blocks |forecolor backcolor| bold italic underline | alignleft aligncenter alignright alignjustify | fullscreen preview code | link image media save',
    default_link_target: '_blank',
    block_unsupported_drop: true,
    entity_encoding: 'raw',
    images_upload_handler: (blobInfo: any) => {
