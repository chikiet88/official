...
export class AuthService {
  ...
  set accessToken(token: string) {
    this._LocalStorageService.setItem('TazagroupToken', token);
  }
  get accessToken(): string {
   return this._LocalStorageService.getItem('TazagroupToken') ?? '';
  }
  ...
  checkDangnhap(): Observable<boolean> {
    ...
    if (!this.accessToken || this.accessToken === 'undefined') {
      this._LocalStorageService.removeItem('TazagroupToken')
      return of(false);
    }
